...
switch ($_GET["op"]){
	...
	case 'listarArticulosVenta':
		...
		$i = 0; 
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				"0"=>'<button id="btndetalleventa'.$i.'" class="btn btn-success" onclick="agregarDetalle('.$reg->idarticulo.',\''.$reg->nombre.'\',\''.$reg->precio_venta.'\',\''.$reg->stock.'\',\''.$i.'\')"><span class="fa fa-check"></span></button>',
 				"1"=>$reg->nombre,
 				"2"=>$reg->descripcion,
 				"3"=>$reg->talle,
 				"4"=>$reg->categoria,
